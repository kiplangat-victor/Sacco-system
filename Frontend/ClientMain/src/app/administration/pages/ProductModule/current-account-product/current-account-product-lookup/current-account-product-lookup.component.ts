...
@Component({
  selector: 'app-current-account-product-lookup',
  templateUrl: './current-account-product-lookup.component.html',
  styleUrls: ['./current-account-product-lookup.component.scss']
})
export class CurrentAccountProductLookupComponent implements OnInit, OnDestroy {
  ...
  getData() {
    this.productsAPI.findCaa().pipe(takeUntil(this.destroy$)).subscribe(
      {
        next: (
          (res) => {
