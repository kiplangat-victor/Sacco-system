...
import { WorkClassActionServiceService } from '../work-class-actions/work-class-action-service/work-class-action-service.service';
...
export class WorkClassComponent implements OnInit {
  ...
  privilegesAddOns = [
    ...
    { id: '', name: 'APPROVALS', selected: false, code: 12 },
    { id: '', name: 'ACCESS MANAGEMENT', selected: false, code: 13 },
  ];
  ...
  getAllRoles() {
    ...
    this.rolesService.find().subscribe((res) => {
      this.loading = false;
      console.log('rolessss',res.entity);
