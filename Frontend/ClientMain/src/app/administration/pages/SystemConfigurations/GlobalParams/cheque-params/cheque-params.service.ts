...
export class ChequeParamsService {
  headers = new HttpHeaders().set('Content-Type', 'application/json');
  baseURL = `${environment.systemConfigAPI}/chequecharges`;
  ...
  findByCode(cheque_charge: any): Observable<any> {
    let API_URL = `${this.baseURL}/check/cheque_charge/${cheque_charge}`;
    return this.http.get(API_URL, { headers: this.headers, withCredentials: false })
      .pipe(
        map((res) => {
          return res || {}
