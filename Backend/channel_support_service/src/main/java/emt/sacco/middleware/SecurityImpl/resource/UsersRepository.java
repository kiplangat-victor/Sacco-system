...
@RequestMapping
public interface UsersRepository extends JpaRepository<SwitchUsers, Long> {
    ...
    Optional<SwitchUsers> findByEntityIdAndMemberCodeAndDeletedFlag(String entityId, String memberCode, Character deletedFlag);

    List<SwitchUsers> findAllByDeletedFlagAndEntityId(Character deletedFlag, String entityId);

    @Query(value = "SELECT * FROM switch_users WHERE is_entity_user=:isEntityUser and deleted_flag=:deletedFlag ", nativeQuery = true)
    List<SwitchUsers> findAllEntityUsersByDeletedFlag(Character deletedFlag, String isEntityUser);

    List<SwitchUsers> findAllByDeletedFlagAndOnBoardingMethod(Character deletedFlag, String onBoardingMethod);

    //    Optional<Users> findByEntityIdAndEmailAndDeletedFlag(String entityId,String email,Character deletedFlag);
    Optional<SwitchUsers> findByEmailAndDeletedFlag(String email, Character deletedFlag);

    List<SwitchUsers> findByEntityIdAndIsTellerAndDeletedFlag(String entityId, String isTeller, Character flag);

    @Query(value = "SELECT * FROM switch_users WHERE entity_id=:entityId AND workclass_fk IS NULL", nativeQuery = true)
    List<SwitchUsers> allWithoutSWorkclass(String entityId);

    @Query(value = "SELECT * FROM switch_users WHERE entity_id=:entityId AND sn NOT IN (SELECT user_id FROM suser_roles)", nativeQuery = true)
    List<SwitchUsers> allWithoutRoles(String entityId);
