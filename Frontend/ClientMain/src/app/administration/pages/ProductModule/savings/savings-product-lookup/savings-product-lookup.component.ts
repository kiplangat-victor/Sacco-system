...
@Component({
  selector: 'app-savings-product-lookup',
  templateUrl: './savings-product-lookup.component.html',
  styleUrls: ['./savings-product-lookup.component.scss']
})
export class SavingsProductLookupComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<SavingsProductLookupComponent>
  ) { }
  ngOnDestroy() {
  ...
  getData() {
    this.productsAPI.findSba().pipe(takeUntil(this.destroy$)).subscribe(
      {
        next: (
          (res) => {
