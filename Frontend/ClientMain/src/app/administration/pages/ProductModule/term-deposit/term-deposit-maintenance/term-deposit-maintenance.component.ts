...
export class TermDepositMaintenanceComponent implements OnInit, OnDestroy {
  params: HttpParams;
  lookupdata: any;
  productCode: any;
  productCodeDesc: any;
  ...
  ngOnInit(): void {
    this.randomCode = "TDA" + Math.floor(Math.random() * (100));
  }
  ...
  onSelectFunction(event: any) {
    ...
    if (event.target.value == 'ADD') {
      ...
      this.formData.controls.productCodeDesc.setValue("Term Depoit Product Description ...");
  ...
  onSubmit() {
    ...
    if (this.formData.valid) {
            ...
            next: (
              (res) => {
                if (res.statusCode === 404) {
                  this.loading = false;
                  this.router.navigate([`/system/product/term-deposit/data/view`], { skipLocationChange: true, queryParams: { formData: this.formData.value } });
                } else if (res.statusCode === 200) {
                  this.loading = false;
                  this.notificationAPI.alertWarning("Term Deposit Product Code Already Registered!");
                }
              }
            ),
            error: (
      ...
      this.notificationAPI.alertWarning("TERM DEPOSIT FORM DATA INVALID");
    }
  }
