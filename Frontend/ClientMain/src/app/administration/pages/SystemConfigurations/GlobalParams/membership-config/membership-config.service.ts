...
export class MembershipConfigService {
  ...
  baseURL = `${environment.systemConfigAPI}/customertype`;
  ...
  findByTypeCode(customer_type: any): Observable<any> {
    let API_URL = `${this.baseURL}/check/customer_type/${customer_type}`;
    return this.http.get(API_URL, { headers: this.headers, withCredentials: false })
      .pipe(
        map((res) => {
          return res || {}
  ...
  findByType(db_table: any): Observable<any> {
    let API_URL = `${this.baseURL}/all/by/table/${db_table}`;
