...
import { AllBranchesLookupComponent } from '../all-branches-lookup/all-branches-lookup.component';
...
export class BranchesMaintenanceComponent implements OnInit, OnDestroy {
  lookupData: any;
  branchCode: any;
 ...
 onSelectFunction(event: any) {
        ...
        this.branchAPI.branchCode(this.branchCode).pipe(takeUntil(this.destroy$)).subscribe(
          (data) => {
            if (data.statusCode === 404) {
              this.loading = false;
              this.router.navigate([`/system/branches/data/view`], { skipLocationChange: true, queryParams: { formData: this.formData.value, fetchData: this.lookupData } });
            } else if (
              data.statusCode === 200) {
              this.loading = false;
  ...
  branchesCodeLookup(): void {
    const dialogRef = this.dialog.open(AllBranchesLookupComponent, {
      width: "40%",
      autoFocus: false,
      maxHeight: '90vh'
    });
